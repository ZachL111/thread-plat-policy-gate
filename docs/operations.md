...
`thread-plat-policy-gate` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for policy analysis with bounded scenario files, conflict explanations, and documented operating limits.
...
The score is deterministic. The current threshold is 181. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 6.
